...
public class CustomGUIPasswordField : CustomGUIControl
{
    ...
    protected override void StyleOffDraw(Rect groupPos)
    {
        ...
        content.text = GUI.PasswordField(guiPos.Pos, content.text, '*');
    ...
    protected override void StyleOnDraw(Rect groupPos)
    {
        if (groupPos != null)
            guiPos.rootRect = groupPos;
        content.text = GUI.PasswordField(guiPos.Pos, content.text, '*', style);
        if (oldStr != content.text)
        {
            textChange?.Invoke(oldStr);
            oldStr = content.text;
